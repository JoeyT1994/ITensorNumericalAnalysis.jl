...
function interpolate_updater(
  ...
  return Π, (; inf_norm_error, updater_time=time)
end
...
function interpolate(f, s::IndsNetworkMap; initial_pivot=random_initial_pivot(s), initial_state = const_itn(s; linkdim = 1), kws...)
  @assert is_tree(s)
  tn, info = interpolate(f, ttn(itensornetwork(initial_state)); initial_pivot, kws...)
  return ITensorNetworkFunction(ITensorNetwork(tn), s), info
end
