...
function default_digit_map(indices::Vector{Index}; map_dimension::Int64=1)
  return Dictionary(
    indices, [ceil(Int64, i / map_dimension) for (i, ind) in enumerate(indices)]
  )
end
function default_dimension_map(indices::Vector{Index}; map_dimension::Int64)
  return Dictionary(indices, [(i % map_dimension) + 1 for (i, ind) in enumerate(indices)])
end

function IndexMap(s::IndsNetwork; map_dimension::Int64=1)
  indices = inds(s)
  return IndexMap(
...
function index_value_to_scalar(im::IndexMap, ind::Index, value::Int64)
...
function ind(im::IndexMap, dimension::Int64, digit::Int64)
  return only(
    filter(
...
function dimension_indices(im::IndexMap, dim::Int64)
...
function calculate_x(im::IndexMap, ind_to_ind_value_map, dimension::Int64)
...
function calculate_ind_values(
  im::IndexMap, xs::Vector{Float64}, dimensions::Vector{Int64}; print_x=false
)
...
function calculate_ind_values(im::IndexMap, x::Float64, dimension::Int64; kwargs...)
  return calculate_ind_values(im, [x], [dimension]; kwargs...)
end
function calculate_ind_values(im::IndexMap, xs::Vector{Float64}; kwargs...)
  return calculate_ind_values(im, xs, [i for i in 1:length(xs)]; kwargs...)
end
function calculate_ind_values(im::IndexMap, x::Float64; kwargs...)
