...
@testset "test operators" begin
    @testset "test differentiation in 1D on MPS" begin
        ...
        for x in xs
            @test 1.0 + evaluate(ψ_fx, x; alg = "exact") ≈ 1.0 + sin(2.0 * pi * x) rtol = 1.0e-3
            @test 1.0 + evaluate(ψ_f1x, x) ≈ 1.0 + 2.0 * pi * cos(2.0 * pi * x) rtol = 1.0e-3
    ...
    @testset "test differentiation in 1D on tree" begin
        ...
        ∂_∂x = first_derivative_operator(g, s; cutoff = 1.0e-10)
        ...
        for x in xs
            ...
            @test ∂x_ψ_fx_x ≈ pi * cos(pi * x) atol = 1.0e-3
        end
    end
    ...
    @testset "test differentiation_operator_on_3D_function" begin
        ...
        ∂_∂y = first_derivative_operator(g, s; dim = 2, cutoff = 1.0e-10)

        ∂_∂y_ψ_fxgyhz = operate([∂_∂y], ψ_fxgyhz; cutoff = 1.0e-10)
        ...
        for x in xs
            for y in ys
                for z in zs
                    ...
                    @test ψ_fxgyhz_xyz ≈ (x^2 - x) * sin(pi * y) * sin(pi * z) atol = 1.0e-3

                    ∂_∂y_ψ_fxgyhz_xyz = real(evaluate(∂_∂y_ψ_fxgyhz, [x, y, z]))
                    @test ∂_∂y_ψ_fxgyhz_xyz ≈ pi * (x^2 - x) * cos(pi * y) * sin(pi * z) atol = 1.0e-3
                end
            end
        end
    end
    ...
    @testset "test multiplication_operator_in_1D" begin
        ...
        for x in xs
            ...
            @test ψ_fxgx_x ≈ sin(0.5 * pi * x) * cos(0.25 * pi * x) atol = 1.0e-3
            ψ_sq_x = real(evaluate(ψ_sq, x))
            @test ψ_sq_x ≈ cos(0.25 * pi * x) * cos(0.25 * pi * x) atol = 1.0e-3
        end
    end
    ...
    @testset "test multiplication_operator_in_2D" begin
        ...
        for x in xs
            for y in ys
                ...
                @test ψ_fxgy_xy ≈ cos(0.25 * pi * x) * sin(0.5 * pi * y) atol = 1.0e-3
                ψ_sq_xy = real(evaluate(ψ_sq, [x, y]))
                @test ψ_sq_xy ≈ (sin(0.5 * pi * y))^2 atol = 1.0e-3
            end
        end
    end
    ...
    @testset "test shift operators in 1D on Tree" begin
        ...
        forward_shift_dirichlet = forward_shift_op(
            g,
            s; boundary = "Dirichlet", truncate_kwargs = (; cutoff = 1.0e-10)
        )
        ...
        forward_shift_pbc = forward_shift_op(
            g,
            s; boundary = "Periodic", truncate_kwargs = (; cutoff = 1.0e-10)
        )
        ...
        forward_shift_neumann = forward_shift_op(
            g,
            s; boundary = "Neumann", truncate_kwargs = (; cutoff = 1.0e-10)
        )
        ...
        ψ_fx_pshift_dirichlet = operate(forward_shift_dirichlet, ψ_fx; cutoff = 1.0e-12)
        ψ_fx_mshift_dirichlet = operate(backward_shift_dirichlet, ψ_fx; cutoff = 1.0e-12)
        ψ_fx_pshift_pbc = operate(forward_shift_pbc, ψ_fx; cutoff = 1.0e-12)
        ψ_fx_mshift_pbc = operate(backward_shift_pbc, ψ_fx; cutoff = 1.0e-12)
        ψ_fx_pshift_neumann = operate(forward_shift_neumann, ψ_fx; cutoff = 1.0e-12)
        ψ_fx_mshift_neumann = operate(backward_shift_neumann, ψ_fx; cutoff = 1.0e-12)

        for x in xs
            if x + delta < 1
                fx_xplus = evaluate(ψ_fx, x + delta)
                @test fx_xplus ≈ evaluate(ψ_fx_pshift_dirichlet, x) atol = 1.0e-8
                @test fx_xplus ≈ evaluate(ψ_fx_pshift_pbc, x) atol = 1.0e-8
                @test fx_xplus ≈ evaluate(ψ_fx_pshift_neumann, x) atol = 1.0e-8
            elseif x == 1.0 - delta
                @test evaluate(ψ_fx_pshift_dirichlet, x) ≈ 0.0 atol = 1.0e-8
                @test evaluate(ψ_fx_pshift_pbc, x) ≈ evaluate(ψ_fx, 0.0) atol = 1.0e-8
                @test evaluate(ψ_fx_pshift_neumann, x) ≈ evaluate(ψ_fx, 1.0 - delta) atol = 1.0e-8
            end

            if x - delta >= 0.0
                fx_xminus = evaluate(ψ_fx, x - delta)
                @test fx_xminus ≈ evaluate(ψ_fx_mshift_dirichlet, x) atol = 1.0e-8
                @test fx_xminus ≈ evaluate(ψ_fx_mshift_pbc, x) atol = 1.0e-8
                @test fx_xminus ≈ evaluate(ψ_fx_mshift_neumann, x) atol = 1.0e-8
            elseif x == 0.0
                @test evaluate(ψ_fx_mshift_dirichlet, x) ≈ 0.0 atol = 1.0e-8
                @test evaluate(ψ_fx_mshift_pbc, x) ≈ evaluate(ψ_fx, 1.0 - delta) atol = 1.0e-8
                @test evaluate(ψ_fx_mshift_neumann, x) ≈ evaluate(ψ_fx, 0.0) atol = 1.0e-8
            end
        end
    end
    ...
    @testset "test double shift operators in 1D on Tree" begin
        ...
        forward_shift_dirichlet = forward_shift_op(
            g,
            s; n, boundary = "Dirichlet", truncate_kwargs = (; cutoff = 1.0e-10)
        )
        backward_shift_dirichlet = backward_shift_op(
            g,
            s; n, boundary = "Dirichlet", truncate_kwargs = (; cutoff = 1.0e-10)
        ...
        forward_shift_pbc = forward_shift_op(
            g,
            s; n, boundary = "Periodic", truncate_kwargs = (; cutoff = 1.0e-10)
        )
        backward_shift_pbc = backward_shift_op(
            g,
            s; n, boundary = "Periodic", truncate_kwargs = (; cutoff = 1.0e-10)
        ...
        forward_shift_neumann = forward_shift_op(
            g,
            s; n, boundary = "Neumann", truncate_kwargs = (; cutoff = 1.0e-10)
        )
        backward_shift_neumann = backward_shift_op(
            g,
            s; n, boundary = "Neumann", truncate_kwargs = (; cutoff = 1.0e-10)
        ...
        for x in xs
            ...
            elseif x == 1.0 - 2.0 * delta || x == 1.0 - delta
                @test evaluate(ψ_fx_pshift_dirichlet, x; alg = "exact") ≈ 0.0 atol = 1.0e-8
                @test evaluate(ψ_fx_pshift_pbc, x) ≈ evaluate(ψ_fx, x + 2.0 * delta - 1.0) atol =
                    1.0e-8
                @test evaluate(ψ_fx_pshift_neumann, x) ≈ evaluate(ψ_fx, x) atol = 1.0e-8
            end
            ...
            if x - 2.0 * delta >= 0.0
                ...
                @test evaluate(ψ_fx_mshift_pbc, x) ≈ evaluate(ψ_fx, x - 2.0 * delta + 1.0) atol =
                    1.0e-8
                @test evaluate(ψ_fx_mshift_neumann, x) ≈ evaluate(ψ_fx, x) atol = 1.0e-8
            end
        end
    end
    ...
    @testset "test shift operators in 2D on Tree" begin
        ...
        forward_shift_dirichlet = forward_shift_op(
            g,
            s; boundary = "Dirichlet", dim = 2, truncate_kwargs = (; cutoff = 1.0e-10)
        )
        backward_shift_dirichlet = backward_shift_op(
            g,
            s; boundary = "Dirichlet", dim = 2, truncate_kwargs = (; cutoff = 1.0e-10)
        ...
        forward_shift_pbc = forward_shift_op(
            g,
            s; boundary = "Periodic", dim = 2, truncate_kwargs = (; cutoff = 1.0e-10)
        )
        backward_shift_pbc = backward_shift_op(
            g,
            s; boundary = "Periodic", dim = 2, truncate_kwargs = (; cutoff = 1.0e-10)
        ...
        forward_shift_neumann = forward_shift_op(
            g,
            s; boundary = "Neumann", dim = 2, truncate_kwargs = (; cutoff = 1.0e-10)
        )
        backward_shift_neumann = backward_shift_op(
            g,
            s; boundary = "Neumann", dim = 2, truncate_kwargs = (; cutoff = 1.0e-10)
        ...
        ψ_fxy_pshift_dirichlet = operate(forward_shift_dirichlet, ψ_fxy; cutoff = 1.0e-12)
        ψ_fxy_mshift_dirichlet = operate(backward_shift_dirichlet, ψ_fxy; cutoff = 1.0e-12)
        ψ_fxy_pshift_pbc = operate(forward_shift_pbc, ψ_fxy; cutoff = 1.0e-12)
        ψ_fxy_mshift_pbc = operate(backward_shift_pbc, ψ_fxy; cutoff = 1.0e-12)
        ψ_fxy_pshift_neumann = operate(forward_shift_neumann, ψ_fxy; cutoff = 1.0e-12)
        ψ_fxy_mshift_neumann = operate(backward_shift_neumann, ψ_fxy; cutoff = 1.0e-12)

        for y in ys
            if y + delta < 1
                fxy_xyplus = evaluate(ψ_fxy, [x, y + delta])
                @test fxy_xyplus ≈ evaluate(ψ_fxy_pshift_dirichlet, [x, y]) atol = 1.0e-8
                @test fxy_xyplus ≈ evaluate(ψ_fxy_pshift_pbc, [x, y]) atol = 1.0e-8
                @test fxy_xyplus ≈ evaluate(ψ_fxy_pshift_neumann, [x, y]) atol = 1.0e-8
            elseif y == 1.0 - delta
                @test evaluate(ψ_fxy_pshift_dirichlet, [x, y]) ≈ 0.0 atol = 1.0e-8
                @test evaluate(ψ_fxy_pshift_pbc, [x, y]) ≈ evaluate(ψ_fxy, [x, 0.0]) atol = 1.0e-8
            ...
            if y - delta >= 0.0
                ...
                @test fxy_xyminus ≈ evaluate(ψ_fxy_mshift_dirichlet, [x, y]) atol = 1.0e-8
                @test fxy_xyminus ≈ evaluate(ψ_fxy_mshift_pbc, [x, y]) atol = 1.0e-8
                @test fxy_xyminus ≈ evaluate(ψ_fxy_mshift_neumann, [x, y]) atol = 1.0e-8
            elseif y == 0.0
                @test evaluate(ψ_fxy_mshift_dirichlet, [x, y]) ≈ 0.0 atol = 1.0e-8
                @test evaluate(ψ_fxy_mshift_pbc, [x, y]) ≈ evaluate(ψ_fxy, [x, 1.0 - delta]) atol =
                    1.0e-8
                @test evaluate(ψ_fxy_mshift_neumann, [x, y]) ≈ evaluate(ψ_fxy, [x, 0.0]) atol = 1.0e-8
            end
        end
    end
    ...
    @testset "test boundary operator in 1D on Tree" begin
        ...
        @testset "boundary apply" begin
            maxdim, cutoff = 10, 1.0e-16
            ϕ_fx = map_to_zeros(ψ_fx, [0, lastDigit]; cutoff, maxdim)
            for x in xs
                val = real(evaluate(ϕ_fx, x))
                @test (x ∈ [0, lastDigit]) ? isapprox(val, 0.0; atol = 1.0e-8) : !(val ≈ 0.0)
            end
        end
    end
    ...
    @testset "test boundary operator in 2D on Tree" begin
        ...
        @testset "boundary apply" begin
            ...
            for x in [0, lastDigit]
                ...
                @test all(isapprox.(vals, 0.0, atol = 1.0e-8))
            end
        end
    end
