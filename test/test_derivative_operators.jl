...
@testset "test differentiation in 1D on MPS" begin
  ...
  ψ_f1x = operate(f1, ψ_fx; truncate_kwargs=(; cutoff=1e-12))
  ψ_f2x = operate(f2, ψ_fx; truncate_kwargs=(; cutoff=1e-12))
  ψ_f3x = operate(f3, ψ_fx; truncate_kwargs=(; cutoff=1e-12))
  ψ_f4x = operate(f4, ψ_fx; truncate_kwargs=(; cutoff=1e-12))

  xs = [0.0, delta, 0.25, 0.625, 0.875, 1.0 - delta]
  for x in xs
    @test calculate_fx(ψ_fx, x) ≈ sin(2.0 * pi * x) atol = 1e-3
    @test calculate_fx(ψ_f1x, x) ≈ 2.0 * pi * cos(2.0 * pi * x) atol = 1e-3
    @test calculate_fx(ψ_f2x, x) ≈ -(2.0 * pi)^(2) *  sin(2.0 * pi * x) atol = 1e-3
    # @test calculate_fx(ψ_f3x, x) ≈ -(2.0 * pi)^(3) *  cos(2.0 * pi * x) atol = 1e-3
    @test calculate_fx(ψ_f4x, x) ≈ (2.0 * pi)^(4) *  sin(2.0 * pi * x) atol = 1e-3
  end
end
