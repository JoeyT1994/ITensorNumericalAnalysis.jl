...
@testset "test real itensorfunctions" begin
    @testset "test constructor from ITensorNetwork" begin
        ...
        fψ = TensorNetworkFunction(ψ, dim_vertices)
    ...
    @testset "test 1D elementary function construction" begin
        @testset "test const" begin
            ...
            # link dims section
            ψ_fx = const_tnf(g, s; c, bond_dimension = 4)
        ...
        @testset "test tanh" begin
            ...
            x = 0.625
            ψ_fx = tanh_tnf(g, s; k, a, c, nterms)
            fx_x = evaluate(ψ_fx, x)
        ...
        @testset "test poly" begin
            ...
            for deg in degrees
                ...
                x = 0.875
                ψ_fx = poly_tnf(g, s, coeffs; k, c)
                fx_x = evaluate(ψ_fx, x)

                fx_exact = c * sum([coeffs[i] * ((k * x)^(i - 1)) for i in 1:(deg + 1)])
                @test fx_x ≈ fx_exact atol = 1.0e-4
            end
        end
    end
    ...
    @testset "test multi-dimensional elementary function construction" begin
        ...
        for (name, net_func, func) in funcs
            @testset "test $name" begin
                ...
                ψ_fx = net_func(g, s; k, a, c, dim = 1)
                ψ_fy = net_func(g, s; k, a, c, dim = 2)
        ...
        @testset "test tanh" begin
            ...
            x, y = 0.625, 0.875
            ψ_fx = tanh_tnf(g, s; k, a, c, nterms, dim = 1)
            ψ_fy = tanh_tnf(g, s; k, a, c, nterms, dim = 2)
